...
export default function ChatBot() {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        // Skapa tomt assistant-meddelande
        ...
        try {
                ...
                while (true) {
                    ...
                    for (const line of lines) {
                        if (line.trim() && line.startsWith('0:')) {
                            try {
                                // Parse line format: 0:"text"
                                const jsonStr = line.substring(2);
                                const text = JSON.parse(jsonStr);
                                assistantContent += text;

                                // Uppdatera assistant-meddelandet
                                setMessages(prev =>
                                    prev.map(m =>
                                        m.id === assistantId
            ...
            // Om ingen content kom igenom, sätt felmeddelande
            if (assistantContent === '') {
                setMessages(prev =>
                    prev.map(m =>
                        m.id === assistantId
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="mb-4 w-[350px] sm:w-[420px] h-[540px] bg-[#0b1220] rounded-2xl shadow-2xl flex flex-col border border-gray-800 overflow-hidden text-white"
                    >
